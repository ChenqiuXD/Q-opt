...
class grad_q_agent:
    ...
    def __init__(self, n_actions, n_features, learning_rate=0.01, reward_decay=0.9, e_greedy=0.9,
        ...
        # td_network calculate the temporal difference of 
        self.td_network = QNetwork(n_features, n_actions, l1, l2)
        self.optimizer_td = torch.optim.SGD(self.td_network.parameters(),
                                            lr=learning_rate*self.gamma)
    ...
    def learn(self):
        ...
        with torch.no_grad():
            q_next_max, max_indices = torch.max( self.q_network(batch_next_state).detach(), dim=1 )
            q_target = batch_reward + self.gamma * ((1-batch_done) * q_next_max.detach()).view(self.batch_size, 1)

        # grad_q update
        next_value = self.q_network(batch_next_state).gather(1, max_indices.view([-1,1]))
        td_error = self.td_network(batch_state).gather(1, batch_action)
        target_next_value = (next_value + td_error).detach()

        loss = self.loss_func(torch.cat([q_eval, next_value] ), torch.cat([q_target, target_next_value]))

        self.optimizer.zero_grad()
        loss.backward()
        self.optimizer.step()

        # td error network update
        td_error_target = (q_target - q_eval).detach()
        loss = self.loss_func( td_error, td_error_target )
